...
if __name__=='__main__':
    ...
    folder = path + "/UFold/data/bpseq_data/"
    ...
    for index,item_file in enumerate(all_files):
        ...
        t0 = subprocess.getstatusoutput('awk \'{print $2}\' '+folder+item_file)
        seq = ''.join(t0[1].split('\n'))
        #pdb_file.write('>%s\n%s\n'%(item_file,seq.upper()))
        ...
        if t0[0] == 0:
            try:
                ...
                pdb.set_trace()
        t1 = subprocess.getstatusoutput('awk \'{print $1}\' '+folder+item_file)
        t2 = subprocess.getstatusoutput('awk \'{print $3}\' '+folder+item_file)
    ...
    ##          end cut sequnce
                ...
                skipped_files.append(item_file)
            
        #pdb.set_trace()
    print(" ")
    print("Total usable files: ")     
    ...
    cPickle.dump(train_set,open(path + "/UFold/data/reduced_saved_data/my_train_set.cPickle","wb"))
    cPickle.dump(test_set,open(path + "/UFold/data/reduced_saved_data/my_test_set.cPickle","wb"))
    cPickle.dump(all_files_list, open(path + "/UFold/data/reduced_saved_data/full_set.cPickle","wb"))
